...
if __name__ == '__main__':
    ...
    def train(data_loader):
        ...
        for batch_idx, samples in bar:
            ...
            model_D.zero_grad()
            inputs_next1, adj_matrix1, distance1 = model_propcess(inputs_next, adj1, v1)
            ...
            errD_real.backward()
            # ------------------------------------------------------------------------
            inputs_start0, adj_matrix0, distance0 = model_propcess(inputs_start, adj, v0)
            U00 = model_PE(inputs_start0)  # noise data
            fake = model_G(U00.squeeze(0), adj_matrix0, distance0)

            U0_fake, adj_matrix_fake, distance_fake = model_propcess(fake.squeeze(0), adj1, v1)
            U0 = model_PE(U0_fake)
            U0 = U0.clone()
            # on fake
            ...
            model_G.zero_grad()
            inputs_start00, adj_matrix, distance0 = model_propcess(inputs_start, adj, v0)
    ...
    if resume_log:
        ...
        train_losses, train_maes = log['train_loss'].to_list(), log['train_mae'].to_list()
        test_maes = log['test_mae'].to_list()
        val_maes = log['val_mae'].to_list()

        epochs = log[:, 0].astype(np.int).tolist()
        best_acc = min(train_maes)
        start_epoch = max(epochs) + 1

        for i, epoch in enumerate(epochs):
            print('\nEpoch: %d' % epoch)
            print('Loss: %.4f |mae: %.4f' % (train_losses[i], train_maes[i]))
            print('mae: %.4f' % (train_maes[i]))
            print('mae: %.4f' % (val_maes[i]))
    ...
    if resume_log:
        ...
        best_mae = min(train_maes)
        best_mae_test=min(test_maes)
        start_epoch = max(epochs) + 1
