...
from .sinkhorn_samples_all import sinkhorn_tensorized
...
class euclidean_distance_wd_no_dis_loss(Module):
    def __init__(
            self,
            loss="sinkhorn",
            p=2,
        ...
        super(euclidean_distance_wd_no_dis_loss, self).__init__()
        self.loss = loss
    ...
    def forward(self, distance, adj, *args, extra_value=True):
        ...
        for i in range(0, infeat_num, self.size):
            ...
            tmp_dis[:, 0, :] = 1.0
            #             print(pred[tmp, :],"----")
            pred_, true_ = pred[tmp, :], true[tmp, :]  # a, b,n   need to recover
    ...
    def generate_weights(self, x, n):
        if x.dim() == 3:
            B, N, _ = x.shape
            tmp = torch.ones(B, N, device=x.device).type_as(x)
            return tmp / n
        else:
            raise ValueError(
