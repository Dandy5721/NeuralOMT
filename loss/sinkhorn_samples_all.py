...
def sigmoid_dis(x,mask, n = 1.2, k = 1.0):#b, n, n
    # print("----------", x.shape, mask.shape)
    sig_dis = torch.exp(n / (1 + torch.exp(-1 * k * x))).detach()
    # print(sig_dis.shape, mask.shape)
    sig_dis = sig_dis * mask
#     print(sig_dis.shape, (sig_dis.sum(1).unsqueeze(1) + 1e-6).shape)
    return sig_dis / (sig_dis.sum(1).unsqueeze(1) + 1e-6)
...
def sinkhorn_tensorized(
    ...
    #     print(cost(x, y.detach()).shape, sigmoid_dis(distance).shape)
    sig_dis = sigmoid_dis(distance.unsqueeze(2) + distance.unsqueeze(1), mask.unsqueeze(2) * mask.unsqueeze(1), k = k)
    if use_dis:
        C_xy = cost(x, y.detach()) * sig_dis  # (B,N,M) torch Tensor
        C_yx = cost(y, x.detach()) * sig_dis
    else:
        C_xy = cost(x, y.detach())
        C_yx = cost(y, x.detach())

    #     C_xy = cost(x, y.detach())
    #     C_yx = cost(y, x.detach())
    #     print("before", C_yx)
    #     C_xy = C_xy * sigmoid_dis(distance.unsqueeze(2)  + distance.unsqueeze(1), mask.unsqueeze(2) * mask.unsqueeze(1))
    #     C_yx = C_yx * sigmoid_dis(distance.unsqueeze(2)  + distance.unsqueeze(1), mask.unsqueeze(2) * mask.unsqueeze(1))
    #     print("after", C_yx)
    # N.B.: The "auto-correlation" matrices C(x_i, x_j) and C(y_i, y_j)
    #       are only used by the "debiased" Sinkhorn algorithm.
    if use_dis:
        C_xx = cost(x, x.detach()) * sig_dis if debias else None  # (B,N,N) torch Tensor
        C_yy = cost(y, y.detach()) * sig_dis if debias else None  # (B,M,M) torch Tensor
    else:
        C_xx = cost(x, x.detach())  if debias else None  # (B,N,N) torch Tensor
        C_yy = cost(y, y.detach())  if debias else None
    ...
    # print("tttt----->", x.min())
    # print("before scaling_parameters---->", x.shape)
    # print("tmp----->", tmp.min())
